    ...
    <x-app-layout>
        ...
        <div class="pt-4 sm:ml-64 mt-16">
            <div class="flex w-full items-center rounded-full">
                ...
                <span class="text-black text-lg font-semibold leading-8 px-8 py-3 uppercase">{{trans('message.product-list')}}</span>
                <div class="flex-1 border-b border-gray-300"></div>
            </div>
            <div class="pl-4 pr-4 border-gray-200 border-dashed rounded-lg dark:border-gray-700">
                <div class="flex justify-between items-center">
                    ...
                    <div class="flex items-center justify-end rounded dark:bg-gray-800">
                        <a href="/admin/product/add_product/view">
                            <button type="button"
    ...
    <script>
        $(document).ready(function() {
            ...
            function loadProduct(page = 1, keyWord) {
                $tableContainer = $('#table-container').empty();
                $.ajax({
                    ...
                    success: function(response) {    
                        ...
                        if (response.status === "success" && Array.isArray(response.data.data)) {
                            response.data.data.forEach(product => {
                                ...
                                <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">${product.price}</td>
                                <td class=" px-5 py-3">
                                    <div class="w-48 flex items-center gap-3">
                                        <div class="data">
                                            <p class="font-normal text-sm text-gray-900">${product.name}</p>
                                        </div>
                                    </div>
                                </td>
                                <td class="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900 ">
                                    <div class="py-1.5 px-2.5 rounded-full flex justify-center w-20 items-center gap-1 
                                        ${product.status == 1 ? 'bg-emerald-50' : 'bg-amber-50'}">
                                        <svg width="5" height="6" viewBox="0 0 5 6" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <circle cx="2.5" cy="3" r="2.5" fill="${product.status == 1 ? '#059669' : '#d97706'}"></circle>
                                        </svg>
                                        <span class="font-medium text-xs ${product.status == 1 ? 'text-emerald-600' : 'text-amber-600'}">
                                            ${product.status == 1 ? 'Active' : 'Cancel'}
                                        </span>
                                    </div>
                                </td>
                                <td class="flex p-5 items-center gap-0.5">
                                    <button class="p-2  rounded-full bg-white group transition-all duration-500 hover:bg-indigo-600 flex item-center">
                                        <a href="/admin/product/edit_product/${product.id}">
                                    ...
                                    <button data-id="${product.id}" id="btn-delete" class="p-2 rounded-full bg-white group transition-all duration-500 hover:bg-red-600 flex item-center">
